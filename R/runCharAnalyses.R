runCharAnalyses <- function(connectionDetails,
                           cdmDatabaseSchema,
                           cdmDatabaseName,
  ...
  for(i in 1:nrow(referenceTable)){
    ...
    if(!dir.exists(plpResultFolder) && !onlyFetchData){
      ...
      if(sum(population$outcomeCount > 0)) {
      
        result <- PatientLevelPrediction:::covariateSummary(plpData = plpData,
                                                            population = population)
        saveRDS(result, file.path(outputFolder, paste0('Analysis_',referenceTable$analysisId[i]),"covSummary.rds"))
      }
